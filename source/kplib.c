	...
	int daglobxoffs, int daglobyoffs)
{
	...
	int x, y, z, xx, yy, zz, *dc = NULL, num, curbits, c, daval, dabits, *hqval, *hqbits, hqcnt, *quanptr = NULL;
	...
	if (*(unsigned short *)kfileptr == SSWAPIB(0xd8ff))
		kfileptr += 2;
	else
		return -1; //"%s is not a JPEG file\n",filename
						...
						for (c = 0; c < lnumcomponents; c++)
						{
							...
							for (yy = 0; yy < (lcompvsamp[c] << 3); yy += 8)
							{
								...
								{  //NOTE: Might help to split this code into firstime vs. refinement (!Ah vs. Ah!=0)
									...
									if (dctbuf)
										dcs = &ldctptr[c][(((y + yy) >> lshy[c])*ldctx[c] + ((x + xx) >> lshx[c])) << 6];
									...
									if (!Ss)
									{
										...
										if (!Ah)
										{
											...
											{
												daval = hufquickval[lcompdc[c]][i];
												curbits -= hufquickbits[lcompdc[c]][i];
											}
											else
											{
										...
										for (; z <= Se; z++)
										{
											...
											else if (zz < 15)
											{
												...
												if (zz)
												{
													...
													curbits -= zz;
													eobrun += ((unsigned)num >> curbits) & pow2mask[zz];
												}
											...
											else
											{
												...
												curbits -= daval;
												v = ((unsigned)num >> curbits) & pow2mask[daval];
